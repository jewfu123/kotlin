            ...
            imageCapture?.let { imageCapture ->
                ...
                imageCapture.takePicture(
                    ...
                    override fun onImageSaved(output: ImageCapture.OutputFileResults) {
                        ...
                        // if file size > limit size then compress
                        Log.v("4444", savedUri.toString())
                        if (1 > 0) {
                            val inputFileName = "/sdcard/Pictures/CameraX Basic/JPEG_document1_2024-05-26-05-23-24-748.jpg";
                            val bbitmap = BitmapFactory.decodeFile(inputFileName)
                            val inputFile = File(inputFileName)
                            //val resultBitmap = compressImage(savedUri.toString())
                            compress(inputFile, bbitmap, 100 * 1024)
//                            val resultBitmap = BitmapFactory.decodeFile(savedUri.toString())
//                            val auxFile: File = File(savedUri.toString())
//                            compress(auxFile, resultBitmap, 100 * 1024)

//                            val fOut: FileOutputStream = FileOutputStream(savedUri.toString())
//
//                            resultBitmap.compress(Bitmap.CompressFormat.JPEG, 10, fOut)
//                            fOut.flush()
//                            fOut.close()
                            Log.v("result 4444", savedUri.toString());
                        } else {
                            Log.v("result 3333", savedUri.toString() + " is less 100KB");
                        }

//                        if (File(savedUri.path).length() > (305 * 1024)) {
//                            var outfile = ImageCompress.scal(savedUri)
//                            Log.v("result","get result compressed filename: " + outfile.toString())
//                        }
                        // ------------------------
                    }
                })
    ...
    private fun compress(outputFile: File, bitmap: Bitmap, targetSize: Long) {
        var factor = 95
        ...
        do {
            ...
            if (factor <= 10) break
        } while (byteArrayOutputStream.size() > targetSize)
